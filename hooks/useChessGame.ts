...
import { playSound } from '../utils/audioUtils.ts';
...
export const useChessGame = (settings: GameSettings) => {
  ...
  const updateGameStatus = useCallback((currentBoard: BoardState, forPlayerColor: PieceColor) => {
    ...
    setKingAnimatingCheckmateCoords(null); // Clear previous checkmate animation trigger
  ...
  const handleResign = useCallback(() => {
    if (gameStatus.winner || gameStatus.isCheckmate || gameStatus.isStalemate || promotingPawn || easterEggFading) return;

    const resigningPlayerColor = currentPlayerColor;
    const winningPlayerColor = resigningPlayerColor === PieceColor.WHITE ? PieceColor.BLACK : PieceColor.WHITE;
    
    const winnerDetails = winningPlayerColor === settings.player1.color ? settings.player1 : settings.player2;

    setGameStatus({
      message: `${winnerDetails.name} (${winnerDetails.color}) wins by resignation!`,
      isCheck: false, // No check in resignation
      isCheckmate: true, // Game is over, considered a win
      isStalemate: false,
      winner: winningPlayerColor,
    });

    playSound(GAME_END_SOUND_DATA_URL);
    setActiveTimerColor(null); // Stop the timer
    setSelectedPieceSquare(null);
    setPossibleMoves([]);
    setKingInCheckSquare(null);
    setKingAnimatingCheckmateCoords(null); // No king animation for resignation
  }, [currentPlayerColor, gameStatus, settings, promotingPawn, easterEggFading]);
  ...
  return {
    ...
    promotePawn,
    resetGame,
    handleResign, // Expose the new function
    isKingInCheckSquare,
    easterEggFading,
    whiteTimeLeft,
    blackTimeLeft,
