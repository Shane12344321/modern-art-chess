...
import CapturedPiecesDisplay from './CapturedPiecesDisplay.tsx';
import { useChessGame } from '../hooks/useChessGame.ts';
import { GameSettings, PieceColor, PieceType, Player, BoardDisplayMode } from '../types.ts';
...
const GameScreen: React.FC<GameScreenProps> = ({ settings, onGoHome }) => {
  const {
    ...
    promotePawn,
    resetGame,
    handleResign, // Get the new resign handler
    isKingInCheckSquare,
    easterEggFading,
    whiteTimeLeft,
    blackTimeLeft,
  ...
  const gameIsOver = gameStatus.isCheckmate || gameStatus.isStalemate || gameStatus.winner;
  const canResign = !gameIsOver && !promotingPawn && !easterEggFading;
  ...
  const gameControls = (
    <>
      <button
        ...
        className="py-2 px-6 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-lg shadow-md transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
      >
        End Game & Go Home
      </button>
      {canResign && (
        <button
          ...
          className="py-2 px-6 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg shadow-md transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Resign
        </button>
      )}
      {gameIsOver && (
         <button
          onClick={resetGame}
          disabled={easterEggFading}
          className="py-2 px-6 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg shadow-md transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Play Again
        </button>
       )}
